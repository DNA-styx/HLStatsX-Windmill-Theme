...
*/
	...
	if ($player) {
		...
		$tblWeapons = new Table(
			array(
				new TableColumn(
					'weapon',
					'Weapon',
					'width=15&type=weaponimg&align=center&link=' . urlencode("mode=weaponinfo&weapon=%k&game=$game&player=$player"),
					$fname
				),
				new TableColumn(
					'modifier',
		...
		// Server-wide weapon statistics (no player context)
		...
		display_page_title('Weapon Statistics');
		display_ingame_menu();
		$tblWeapons->draw($result, $db->num_rows($result), 100);

	} else {
		error('No player ID or game specified.');
	}
?>